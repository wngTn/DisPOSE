"""
Utility functions for the Shelf dataset
"""
...
class GTDataLoader:
    """Loader for ground truth data from Shelf dataset."""
    ...
    def __init__(
        ...
        self.sequences = sequences  # For Shelf, this is always ["defalta"]
        self.interval = interval
        self.frame_interval = frame_interval
        self.root_idx = root_idx

        self.num_joints = 14  # Shelf convention
    ...
    def _get_frame_list(self) -> list[int]:
        ...
        if self.frame_interval is not None:
            ...
            raise ValueError("frame_interval must be provided for Shelf dataset.")
    ...
    def _process_frame(
        ...
        # Person IDs are scene-global in Shelf; broadcast across views to match
        # MMORDataset / PanopticDataset shape (V, N) so the downstream code that
        # indexes by (B, A, T, V, N) sees the right rank.
        person_ids = person_ids[None].repeat(cam_params.shape[0], 0)
        # global_ids stays 1-D (N,) to match panoptic/mm_or/campus —
        # RootMetrics slices as batch["global_ids"][:, 0, t_mid] expecting (B, N).
        ...
        for cam_idx in self.cam_list:
            img_path = self.data_root / f"Camera{cam_idx}" / f"img_{frame_num:06d}.png"
    ...
    def __call__(self) -> tuple[dict, list[tuple]]:
        ...
        # Shelf has single sequence "defalta"
        sequence = "defalta"
...
class PseudoGTDataLoader:
    ...
    def _process_frame(
        ...
        # Camera keys for Shelf are integers
        ...
        # person_ids broadcast to (V, N) for align_pred_to_gt's view-aware
        # rearrange. global_ids stays 1-D (N,) — RootMetrics slices by
        # batch["global_ids"][:, 0, t_mid] and expects (B, N).
        person_ids_v = person_ids[None].repeat(self.num_cams, 0)
        global_ids_v = np.arange(global_id_counter, global_id_counter + total_people)
        ...
        return {
            ...
            "gt_keypoints_xyzs": gt_keypoints_xyzs,
            "gt_keypoints_xys": gt_keypoints_xys,
            "person_ids": person_ids_v,
            "global_ids": global_ids_v,
            "img_paths": img_paths,
            "cam_params_vec": cam_params,
...
def get_cam_params(sequences: list[str], data_root: Path, cam_list: list[int]):
    """
    Initialize Camera Parameters for Shelf dataset.

    Note: Shelf uses a different camera parameter convention than Panoptic.
    - T in calibration file is the camera center (C), not translation vector
    - No coordinate transform matrix M is applied
    """
    cam_params = {}

    cal_path = data_root / "calibration_shelf.json"
    ...
    for cam_idx in cam_list:
        ...
        # T in Shelf calibration is Camera Center (C), not translation vector (t)
